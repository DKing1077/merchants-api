...
def create_payment_intent_route(request: CreatePaymentIntentRequest, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"),
...
@router.get("/")
def list_payment_intents_route(limit: int = Query(default=20, ge=1, le=100), starting_after: str | None = Query(default=None),
    status: str | None = Query(default=None), created_after: datetime | None = Query(default=None),
    created_before: datetime | None = Query(default=None), currency: str | None = Query(default=None),
    db=Depends(get_db), api_key=Depends(require_api_key)):
...
def list_transactions_route(limit: int = Query(default=20, ge=1, le=100), starting_after: str | None = Query(default=None),
    db=Depends(get_db), api_key=Depends(require_api_key)):
...
def list_flagged_payment_intents_route(limit: int = Query(default=20, ge=1, le=100), starting_after: str | None = Query(default=None),
...
def flag_payment_intent_route(payment_intent_id: str, request: FlagPaymentIntentRequest,
    db=Depends(get_db), api_key=Depends(require_admin_api_key)):
    try:
...
def review_payment_intent_route(payment_intent_id: str, request: ReviewPaymentIntentRequest,
    db=Depends(get_db), api_key=Depends(require_admin_api_key)):
    try:
...
def capture_payment_intent_route(payment_intent_id: str, request: CapturePaymentIntentRequest | None = None,
    db=Depends(get_db), api_key=Depends(require_api_key)):
    try:
...
def list_payment_intent_transactions_route(payment_intent_id: str, limit: int = Query(default=20, ge=1, le=100),
    starting_after: str | None = Query(default=None), db=Depends(get_db), api_key=Depends(require_api_key)):
    try:
